import { a8 as computeDimensionOfText, l as log, a9 as line, m as setAccDescription, n as getAccDescription, D as getDiagramTitle, C as setDiagramTitle, o as getAccTitle, q as setAccTitle, I as clear$1, Q as getConfig, P as cleanAndMerge, R as config, a7 as getThemeVariables, r as sanitizeText, a2 as selectSvgElement, x as configureSvgSize } from "./mermaid-BTN5MP6C.js";
...
class BandAxis extends BaseAxis {
  ...
  getScaleValue(value) {
    var _a;
    return (_a = this.scale(value)) !== null && _a !== void 0 ? _a : this.getRange()[0];
  }
}
...
class XYChartBuilder {
  static build(config2, chartData, chartThemeConfig, tmpSVGGroup2) {
    const orchestrator = new Orchestrator(config2, chartData, chartThemeConfig, tmpSVGGroup2);
...
function getChartDefaultThemeConfig() {
  ...
  return cleanAndMerge(defaultThemeVariables.xyChart, config2.themeVariables.xyChart);
}
function getChartDefaultConfig() {
  const config$1 = getConfig();
  return cleanAndMerge(config.xyChart, config$1.xyChart);
}
...
function textSanitizer(text) {
  const config2 = getConfig();
  return sanitizeText(text.trim(), config2);
}
...
const draw = (txt, id, _version, diagObj) => {
  ...
  const labelData = db2.getXYChartData().plots[0].data.map((data) => data[1]);
      ...
      case "rect":
        ...
        if (chartConfig.showDataLabel) {
          if (chartConfig.chartOrientation === "horizontal") {
            let fitsHorizontally = function(item, fontSize) {
            ...
            let fitsInBar = function(item, fontSize, yOffset2) {
              const { data, label } = item;
              const charWidthFactor = 0.7;
              const textWidth = fontSize * label.length * charWidthFactor;
