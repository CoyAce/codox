...
import { n as getAccDescription, m as setAccDescription, o as getAccTitle, q as setAccTitle, D as getDiagramTitle, C as setDiagramTitle, R as config, P as cleanAndMerge, Q as getConfig$1, I as clear$1, l as log, $ as getDefaultExportFromCjs, bs as unknownIcon, aW as createText, g as getConfig$2, bt as getIconSVG, a2 as selectSvgElement, aF as setupGraphViewbox, w as select, bu as registerIconPacks } from "./mermaid-BTN5MP6C.js";
...
import { c as cytoscape$1 } from "./cytoscape.esm-BYzC5ZcP.js";
...
const ArchitectureDirectionArrow = {
  L: (scale) => `${scale},${scale / 2} 0,${scale} 0,0`,
  R: (scale) => `0,${scale / 2} ${scale},0 ${scale},${scale}`,
  T: (scale) => `0,0 ${scale},0 ${scale / 2},${scale}`,
  B: (scale) => `${scale / 2},0 ${scale},${scale} 0,${scale}`
};
const ArchitectureDirectionArrowShift = {
  L: (orig, arrowSize) => orig - arrowSize + 2,
  R: (orig, _arrowSize) => orig - 2,
  T: (orig, arrowSize) => orig - arrowSize + 2,
  B: (orig, _arrowSize) => orig - 2
};
const getOppositeArchitectureDirection = function(x) {
...
const isArchitectureDirection = function(x) {
  const temp = x;
  return temp === "L" || temp === "R" || temp === "T" || temp === "B";
};
const isArchitectureDirectionX = function(x) {
  const temp = x;
  return temp === "L" || temp === "R";
};
const isArchitectureDirectionY = function(x) {
  const temp = x;
  return temp === "T" || temp === "B";
};
const isArchitectureDirectionXY = function(a, b) {
  const aX_bY = isArchitectureDirectionX(a) && isArchitectureDirectionY(b);
  const aY_bX = isArchitectureDirectionY(a) && isArchitectureDirectionX(b);
  return aX_bY || aY_bX;
};
const isArchitecturePairXY = function(pair) {
  const lhs = pair[0];
  const rhs = pair[1];
...
const isValidArchitectureDirectionPair = function(x) {
  return x !== "LL" && x !== "RR" && x !== "TT" && x !== "BB";
};
const getArchitectureDirectionPair = function(sourceDir, targetDir) {
  const pair = `${sourceDir}${targetDir}`;
  return isValidArchitectureDirectionPair(pair) ? pair : void 0;
};
const shiftPositionByArchitectureDirectionPair = function([x, y], pair) {
  const lhs = pair[0];
  const rhs = pair[1];
...
const getArchitectureDirectionXYFactors = function(pair) {
  if (pair === "LT" || pair === "TL") {
    return [1, 1];
...
const getArchitectureDirectionAlignment = function(a, b) {
...
const isArchitectureService = function(x) {
  const temp = x;
  return temp.type === "service";
};
const isArchitectureJunction = function(x) {
  const temp = x;
...
const DEFAULT_ARCHITECTURE_CONFIG = config.architecture;
...
const addService = function({ id, icon, in: parent, title, iconText }) {
...
const getServices = () => Object.values(state.records.nodes).filter(isArchitectureService);
const addJunction = function({ id, in: parent }) {
  state.records.registeredIds[id] = "node";
  state.records.nodes[id] = {
    id,
    type: "junction",
...
const getJunctions = () => Object.values(state.records.nodes).filter(isArchitectureJunction);
const getNodes = () => Object.values(state.records.nodes);
const getNode = (id) => state.records.nodes[id];
const addGroup = function({ id, icon, in: parent, title }) {
  if (state.records.registeredIds[id] !== void 0) {
...
const addEdge = function({ lhsId, rhsId, lhsDir, rhsDir, lhsInto, rhsInto, lhsGroup, rhsGroup, title }) {
  ...
  if (state.records.dataStructures === void 0) {
    ...
    const adjList = Object.entries(state.records.nodes).reduce((prevOuter, [id, service]) => {
      prevOuter[id] = service.edges.reduce((prevInner, edge) => {
        var _a, _b, _c, _d;
        const lhsGroupId = (_a = getNode(edge.lhsId)) === null || _a === void 0 ? void 0 : _a.in;
        const rhsGroupId = (_b = getNode(edge.rhsId)) === null || _b === void 0 ? void 0 : _b.in;
        if (lhsGroupId && rhsGroupId && lhsGroupId !== rhsGroupId) {
          const alignment = getArchitectureDirectionAlignment(edge.lhsDir, edge.rhsDir);
          if (alignment !== "bend") {
            (_c = groupAlignments[lhsGroupId]) !== null && _c !== void 0 ? _c : groupAlignments[lhsGroupId] = {};
            groupAlignments[lhsGroupId][rhsGroupId] = alignment;
            (_d = groupAlignments[rhsGroupId]) !== null && _d !== void 0 ? _d : groupAlignments[rhsGroupId] = {};
...
const getElementById = (id) => state.records.elements[id];
const getConfig = () => {
  const config2 = cleanAndMerge({
    ...DEFAULT_ARCHITECTURE_CONFIG,
    ...getConfig$1().architecture
  });
  return config2;
};
const db = {
...
const drawEdges = async function(edgesEl, cy) {
  ...
  await Promise.all(cy.edges().map(async (edge) => {
    ...
    if (!sourceGroup && ((_a = db.getNode(source)) === null || _a === void 0 ? void 0 : _a.type) === "junction") {
      if (isArchitectureDirectionX(sourceDir)) {
    ...
    if (!targetGroup && ((_b = db.getNode(target)) === null || _b === void 0 ? void 0 : _b.type) === "junction") {
      if (isArchitectureDirectionX(targetDir)) {
...
const drawGroups = async function(groupsEl, cy) {
  const padding = getConfigField("padding");
  ...
  await Promise.all(cy.nodes().map(async (node) => {
    ...
    if (data.type === "group") {
      ...
      if (data.icon) {
        ...
        bkgElem.html(`<g>${await getIconSVG(data.icon, { height: groupIconSize, width: groupIconSize, fallbackPrefix: architectureIcons.prefix })}</g>`);
...
const drawServices = async function(db2, elem, services) {
  var _a;
  for (const service of services) {
    ...
    if (service.icon) {
      bkgElem.html(`<g>${await getIconSVG(service.icon, { height: iconSize, width: iconSize, fallbackPrefix: architectureIcons.prefix })}</g>`);
    } else if (service.iconText) {
      bkgElem.html(`<g>${await getIconSVG("blank", { height: iconSize, width: iconSize, fallbackPrefix: architectureIcons.prefix })}</g>`);
      ...
      const fontSize = (_a = parseInt(window.getComputedStyle(divElem.node(), null).getPropertyValue("font-size").replace(/\D/g, ""))) !== null && _a !== void 0 ? _a : 16;
...
const drawJunctions = function(db2, elem, junctions) {
...
function getAlignments(db2, spatialMaps, groupAlignments) {
    ...
    return Object.entries(alignmentObj).reduce((prev, [dir, alignments2]) => {
      ...
      for (let i = 0; i < arr.length - 1; i++) {
        for (let j = i + 1; j < arr.length; j++) {
          ...
          const alignment = (_a = groupAlignments[aGroupId]) === null || _a === void 0 ? void 0 : _a[bGroupId];
          if (alignment === alignmentDir) {
            (_b = prev[dir]) !== null && _b !== void 0 ? _b : prev[dir] = [];
            prev[dir] = [...prev[dir], ...aNodeIds, ...bNodeIds];
          } else if (aGroupId === "default" || bGroupId === "default") {
            (_c = prev[dir]) !== null && _c !== void 0 ? _c : prev[dir] = [];
            prev[dir] = [...prev[dir], ...aNodeIds, ...bNodeIds];
          } else {
            const keyA = `${dir}-${cnt++}`;
            prev[keyA] = aNodeIds;
  ...
  const alignments = spatialMaps.map((spatialMap) => {
    ...
    Object.entries(spatialMap).forEach(([id, [x, y]]) => {
      ...
      var _g, _h;
      const nodeGroup = (_b = (_a = db2.getNode(id)) === null || _a === void 0 ? void 0 : _a.in) !== null && _b !== void 0 ? _b : "default";
      (_c = horizontalAlignments[y]) !== null && _c !== void 0 ? _c : horizontalAlignments[y] = {};
      (_d = (_g = horizontalAlignments[y])[nodeGroup]) !== null && _d !== void 0 ? _d : _g[nodeGroup] = [];
      horizontalAlignments[y][nodeGroup].push(id);
      (_e = verticalAlignments[x]) !== null && _e !== void 0 ? _e : verticalAlignments[x] = {};
      (_f = (_h = verticalAlignments[x])[nodeGroup]) !== null && _f !== void 0 ? _f : _h[nodeGroup] = [];
...
function getRelativeConstraints(spatialMaps) {
  ...
  const posToStr = (pos) => `${pos[0]},${pos[1]}`;
  const strToPos = (pos) => pos.split(",").map((p) => parseInt(p));
  ...
  return new Promise((resolve) => {
    ...
    layout.one("layoutstop", () => {
      ...
      for (const edge of Object.values(cy.edges())) {
        if ((_a = edge.data) === null || _a === void 0 ? void 0 : _a.call(edge)) {
