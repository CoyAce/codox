...
import { D as getDiagramTitle, C as setDiagramTitle, m as setAccDescription, n as getAccDescription, o as getAccTitle, q as setAccTitle, l as log, I as clear$2, v as common, P as cleanAndMerge, Q as getConfig$1, R as config, S as random, g as getConfig$2, w as select, H as utils, T as setupGraphViewbox } from "./mermaid-DiDd0-cy.js";
import { p as populateCommonDb } from "./populateCommonDb-DR5KFGlj.js";
import { I as ImperativeState } from "./imperativeState-CzGTFSDU.js";
const commitType = {
  NORMAL: 0,
  REVERSE: 1,
  HIGHLIGHT: 2,
...
const DEFAULT_GITGRAPH_CONFIG$1 = config.gitGraph;
const getConfig = () => {
  const config2 = cleanAndMerge({
    ...DEFAULT_GITGRAPH_CONFIG$1,
    ...getConfig$1().gitGraph
  });
  return config2;
};
const state = new ImperativeState(() => {
  const config2 = getConfig();
  const mainBranchName = config2.mainBranchName;
  const mainBranchOrder = config2.mainBranchOrder;
  return {
    mainBranchName,
...
const commit = function(commitDB) {
  ...
  const config2 = getConfig();
  id = common.sanitizeText(id, config2);
  msg = common.sanitizeText(msg, config2);
  tags = tags === null || tags === void 0 ? void 0 : tags.map((tag) => common.sanitizeText(tag, config2));
  const newCommit = {
  ...
  log.info("main branch", config2.mainBranchName);
  state.records.commits.set(newCommit.id, newCommit);
  state.records.branches.set(state.records.currBranch, newCommit.id);
  log.debug("in pushCommit " + newCommit.id);
};
const branch = function(branchDB) {
...
const merge = (mergeDB) => {
  ...
  const config2 = getConfig();
  otherBranch = common.sanitizeText(otherBranch, config2);
  if (customId) {
    customId = common.sanitizeText(customId, config2);
  }
  ...
  if (customId && state.records.commits.has(customId)) {
    ...
    error.hash = {
      text: `merge ${otherBranch} ${customId} ${overrideType} ${customTags === null || customTags === void 0 ? void 0 : customTags.join(" ")}`,
      token: `merge ${otherBranch} ${customId} ${overrideType} ${customTags === null || customTags === void 0 ? void 0 : customTags.join(" ")}`,
      expected: [
        `merge ${otherBranch} ${customId}_UNIQUE ${overrideType} ${customTags === null || customTags === void 0 ? void 0 : customTags.join(" ")}`
      ]
    };
    throw error;
  }
...
const cherryPick = function(cherryPickDB) {
  ...
  const config2 = getConfig();
  sourceId = common.sanitizeText(sourceId, config2);
  targetId = common.sanitizeText(targetId, config2);
  tags = tags === null || tags === void 0 ? void 0 : tags.map((tag) => common.sanitizeText(tag, config2));
  parentCommitId = common.sanitizeText(parentCommitId, config2);
  ...
  if (!targetId || !state.records.commits.has(targetId)) {
    ...
    const commit2 = {
      ...
      message: `cherry-picked ${sourceCommit === null || sourceCommit === void 0 ? void 0 : sourceCommit.message} into ${state.records.currBranch}`,
      seq: state.records.seq++,
...
const checkout = function(branch2) {
  ...
  if (!state.records.branches.has(branch2)) {
    ...
    if (id === void 0 || !id) {
      ...
      state.records.head = (_a = state.records.commits.get(id)) !== null && _a !== void 0 ? _a : null;
    }
  }
};
...
const parseStatement = (statement, db2) => {
  const parsers = {
    Commit: (stmt) => db2.commit(parseCommit(stmt)),
    Branch: (stmt) => db2.branch(parseBranch(stmt)),
    Merge: (stmt) => db2.merge(parseMerge(stmt)),
    Checkout: (stmt) => db2.checkout(parseCheckout(stmt)),
    CherryPicking: (stmt) => db2.cherryPick(parseCherryPicking(stmt))
  };
...
const parseCommit = (commit2) => {
  var _a, _b;
  const commitDB = {
    id: commit2.id,
    msg: (_a = commit2.message) !== null && _a !== void 0 ? _a : "",
    type: commit2.type !== void 0 ? commitType[commit2.type] : commitType.NORMAL,
    tags: (_b = commit2.tags) !== null && _b !== void 0 ? _b : void 0
  };
  return commitDB;
};
const parseBranch = (branch2) => {
  var _a;
  const branchDB = {
    name: branch2.name,
    order: (_a = branch2.order) !== null && _a !== void 0 ? _a : 0
  };
  return branchDB;
};
const parseMerge = (merge2) => {
  var _a, _b;
  const mergeDB = {
    branch: merge2.branch,
    id: (_a = merge2.id) !== null && _a !== void 0 ? _a : "",
    type: merge2.type !== void 0 ? commitType[merge2.type] : void 0,
    tags: (_b = merge2.tags) !== null && _b !== void 0 ? _b : void 0
  };
  return mergeDB;
};
const parseCheckout = (checkout2) => {
  const branch2 = checkout2.branch;
  return branch2;
};
const parseCherryPicking = (cherryPicking) => {
  var _a;
  const cherryPickDB = {
    id: cherryPicking.id,
    targetId: "",
    tags: ((_a = cherryPicking.tags) === null || _a === void 0 ? void 0 : _a.length) === 0 ? void 0 : cherryPicking.tags,
...
const DEFAULT_CONFIG = getConfig$2();
const DEFAULT_GITGRAPH_CONFIG = DEFAULT_CONFIG === null || DEFAULT_CONFIG === void 0 ? void 0 : DEFAULT_CONFIG.gitGraph;
const LAYOUT_OFFSET = 10;
const COMMIT_STEP = 40;
const PX = 4;
const PY = 2;
const THEME_COLOR_LIMIT = 8;
const branchPos = /* @__PURE__ */ new Map();
const commitPos = /* @__PURE__ */ new Map();
const defaultPos = 30;
let allCommitsDict = /* @__PURE__ */ new Map();
let lanes = [];
let maxPos = 0;
let dir = "LR";
const clear = () => {
...
const findClosestParent = (parents) => {
  ...
  parents.forEach((parent) => {
    var _a, _b;
    const parentPosition = dir === "TB" || dir == "BT" ? (_a = commitPos.get(parent)) === null || _a === void 0 ? void 0 : _a.y : (_b = commitPos.get(parent)) === null || _b === void 0 ? void 0 : _b.x;
...
const setParallelBTPos = (sortedKeys, commits, defaultPos2) => {
...
const findClosestParentPos = (commit2) => {
  var _a;
...
const setRootPosition = (commit2, curPos, defaultPos2) => {
  const branch2 = branchPos.get(commit2.branch);
  if (!branch2) {
    throw new Error(`Branch not found for commit ${commit2.id}`);
  }
...
const drawCommitBullet = (gBullets, commit2, commitPosition, typeClass, branchIndex, commitSymbolType) => {
...
const drawCommitLabel = (gLabels, commit2, commitPosition, pos) => {
  var _a;
  if (commit2.type !== commitType.CHERRY_PICK && (commit2.customId && commit2.type === commitType.MERGE || commit2.type !== commitType.MERGE) && (DEFAULT_GITGRAPH_CONFIG === null || DEFAULT_GITGRAPH_CONFIG === void 0 ? void 0 : DEFAULT_GITGRAPH_CONFIG.showCommitLabel)) {
    ...
    const bbox = (_a = text.node()) === null || _a === void 0 ? void 0 : _a.getBBox();
    if (bbox) {
...
const drawCommitTags = (gLabels, commit2, commitPosition, pos) => {
  var _a;
  if (commit2.tags.length > 0) {
    ...
    for (const tagValue of commit2.tags.reverse()) {
      ...
      const tagBbox = (_a = tag.node()) === null || _a === void 0 ? void 0 : _a.getBBox();
      if (!tagBbox) {
...
const getCommitClassType = (commit2) => {
  var _a;
  const commitSymbolType = (_a = commit2.customType) !== null && _a !== void 0 ? _a : commit2.type;
...
const calculatePosition = (commit2, dir2, pos, commitPos2) => {
  var _a, _b, _c;
  const defaultCommitPosition = { x: 0, y: 0 };
  if (commit2.parents.length > 0) {
    const closestParent = findClosestParent(commit2.parents);
    if (closestParent) {
      const parentPosition = (_a = commitPos2.get(closestParent)) !== null && _a !== void 0 ? _a : defaultCommitPosition;
      if (dir2 === "TB") {
        return parentPosition.y + COMMIT_STEP;
      } else if (dir2 === "BT") {
        const currentPosition = (_b = commitPos2.get(commit2.id)) !== null && _b !== void 0 ? _b : defaultCommitPosition;
    ...
    if (dir2 === "TB") {
      ...
      const currentPosition = (_c = commitPos2.get(commit2.id)) !== null && _c !== void 0 ? _c : defaultCommitPosition;
      return currentPosition.y - COMMIT_STEP;
    } else {
      return 0;
    }
...
const getCommitPosition = (commit2, pos, isParallelCommits) => {
  var _a, _b;
  const posWithOffset = dir === "BT" && isParallelCommits ? pos : pos + LAYOUT_OFFSET;
  const y = dir === "TB" || dir === "BT" ? posWithOffset : (_a = branchPos.get(commit2.branch)) === null || _a === void 0 ? void 0 : _a.pos;
  const x = dir === "TB" || dir === "BT" ? (_b = branchPos.get(commit2.branch)) === null || _b === void 0 ? void 0 : _b.pos : posWithOffset;
...
const drawCommits = (svg, commits, modifyGraph) => {
  ...
  const isParallelCommits = (_a = DEFAULT_GITGRAPH_CONFIG === null || DEFAULT_GITGRAPH_CONFIG === void 0 ? void 0 : DEFAULT_GITGRAPH_CONFIG.parallelCommits) !== null && _a !== void 0 ? _a : false;
  const sortKeys = (a, b) => {
    var _a2, _b;
    const seqA = (_a2 = commits.get(a)) === null || _a2 === void 0 ? void 0 : _a2.seq;
    const seqB = (_b = commits.get(b)) === null || _b === void 0 ? void 0 : _b.seq;
  ...
  sortedKeys.forEach((key) => {
    ...
    if (modifyGraph) {
      ...
      const commitSymbolType = (_a2 = commit2.customType) !== null && _a2 !== void 0 ? _a2 : commit2.type;
      const branchIndex = (_c = (_b = branchPos.get(commit2.branch)) === null || _b === void 0 ? void 0 : _b.index) !== null && _c !== void 0 ? _c : 0;
...
const drawArrow = (svg, commitA, commitB, allCommits) => {
  ...
  let radius = 0;
  let offset = 0;
  let colorClassNum = (_a = branchPos.get(commitB.branch)) === null || _a === void 0 ? void 0 : _a.index;
  if (commitB.type === commitType.MERGE && commitA.id !== commitB.parents[0]) {
    colorClassNum = (_b = branchPos.get(commitA.branch)) === null || _b === void 0 ? void 0 : _b.index;
  }
  let lineDef;
  if (arrowNeedsRerouting) {
    ...
    if (dir === "TB") {
      if (p1.x < p2.x) {
        ...
        colorClassNum = (_c = branchPos.get(commitA.branch)) === null || _c === void 0 ? void 0 : _c.index;
      ...
      if (p1.x < p2.x) {
        ...
        colorClassNum = (_d = branchPos.get(commitA.branch)) === null || _d === void 0 ? void 0 : _d.index;
        lineDef = `M ${p1.x} ${p1.y} L ${lineX + radius} ${p1.y} ${arc2} ${lineX} ${p1.y - offset} L ${lineX} ${p2.y + radius} ${arc} ${lineX - offset} ${p2.y} L ${p2.x} ${p2.y}`;
      }
    } else {
      if (p1.y < p2.y) {
        ...
        colorClassNum = (_e = branchPos.get(commitA.branch)) === null || _e === void 0 ? void 0 : _e.index;
...
const drawBranches = (svg, branches) => {
  ...
  branches.forEach((branch2, index) => {
    ...
    const pos = (_a = branchPos.get(branch2.name)) === null || _a === void 0 ? void 0 : _a.pos;
    if (pos === void 0) {
    ...
    bkg.attr("class", "branchLabelBkg label" + adjustIndexForTheme).attr("rx", 4).attr("ry", 4).attr("x", -bbox.width - 4 - ((DEFAULT_GITGRAPH_CONFIG === null || DEFAULT_GITGRAPH_CONFIG === void 0 ? void 0 : DEFAULT_GITGRAPH_CONFIG.rotateCommitLabel) === true ? 30 : 0)).attr("y", -bbox.height / 2 + 8).attr("width", bbox.width + 18).attr("height", bbox.height + 4);
    label.attr("transform", "translate(" + (-bbox.width - 14 - ((DEFAULT_GITGRAPH_CONFIG === null || DEFAULT_GITGRAPH_CONFIG === void 0 ? void 0 : DEFAULT_GITGRAPH_CONFIG.rotateCommitLabel) === true ? 30 : 0)) + ", " + (pos - bbox.height / 2 - 1) + ")");
    if (dir === "TB") {
...
const setBranchPosition = function(name, pos, index, bbox, rotateCommitLabel) {
...
const draw = function(txt, id, ver, diagObj) {
  ...
  const rotateCommitLabel = (_a = DEFAULT_GITGRAPH_CONFIG.rotateCommitLabel) !== null && _a !== void 0 ? _a : false;
  ...
  branches.forEach((branch2, index) => {
    ...
    (_a2 = label.node()) === null || _a2 === void 0 ? void 0 : _a2.appendChild(labelElement);
    const bbox = labelElement.getBBox();
  ...
  utils.insertTitle(diagram2, "gitTitleText", (_b = DEFAULT_GITGRAPH_CONFIG.titleTopMargin) !== null && _b !== void 0 ? _b : 0, db2.getDiagramTitle());
  setupGraphViewbox(void 0, diagram2, DEFAULT_GITGRAPH_CONFIG.diagramPadding, DEFAULT_GITGRAPH_CONFIG.useMaxWidth);
};
