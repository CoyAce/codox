...
class StreamImpl {
  ...
  indexOf(searchElement, fromIndex = 0) {
    ...
    let next = iterator.next();
    while (!next.done) {
      if (index2 >= fromIndex && next.value === searchElement) {
        return index2;
      }
      next = iterator.next();
      index2++;
...
function getPreviousNode(node, hidden = true) {
  while (node.container) {
    ...
    let index2 = parent.content.indexOf(node);
    while (index2 > 0) {
      index2--;
      const previous = parent.content[index2];
      ...
      if (!property2.startsWith("$")) {
        ...
        if (isAstNode(value)) {
          ...
          while (state.arrayIndex < value.length) {
            const index2 = state.arrayIndex++;
            const element = value[index2];
        ...
        if (isReference(value)) {
          ...
          while (state.arrayIndex < value.length) {
            ...
            if (isReference(element)) {
              return { done: false, value: { reference: element, container: node, property: property2, index: index2 } };
            }
          }
          state.arrayIndex = 0;
        }
...
function findNodeForProperty(node, property2, index2) {
  if (!node || !property2) {
    return void 0;
  }
  ...
  if (index2 !== void 0) {
    index2 = Math.max(0, Math.min(index2, nodes.length - 1));
  } else {
    index2 = 0;
  }
  return nodes[index2];
...
function findNodeForKeyword(node, keyword, index2) {
  if (!node) {
    return void 0;
  }
...
function baseDifference(array, values2, iteratee, comparator) {
  var index2 = -1, includes2 = arrayIncludes, isCommon = true, length = array.length, result = [], valuesLength = values2.length;
  if (!length) {
    return result;
  }
...
class PredicateSet {
  ...
  is(index2) {
    return index2 >= this.predicates.length || this.predicates[index2];
  }
  set(index2, value) {
    this.predicates[index2] = value;
  }
  toString() {
    let value = "";
  ...
  for (const alt of sequences) {
    ...
    for (const tokType of alt) {
      ...
      for (const index2 of indices) {
        if (fullSet.has(index2)) {
          if (!altSet.has(index2)) {
            return false;
          }
        } else {
          fullSet.add(index2);
...
class CstNodeBuilder {
  ...
  removeNode(node) {
    const parent = node.container;
    if (parent) {
      const index2 = parent.content.indexOf(node);
      if (index2 >= 0) {
        parent.content.splice(index2, 1);
      }
    }
  }
  addHiddenNodes(tokens) {
    ...
    while (current.container) {
      const index2 = current.container.content.indexOf(current);
      if (index2 > 0) {
        current.container.content.splice(index2, 0, ...nodes);
        added = true;
        break;
      }
...
class LangiumCompletionParser extends AbstractLangiumParser {
  ...
  after(element) {
    if (!this.isRecording()) {
      const index2 = this.elementStack.lastIndexOf(element);
      if (index2 >= 0) {
        this.elementStack.splice(index2);
      }
    }
  }
  get currIdx() {
  ...
  if (isParserRule(element)) {
    ...
    while (!isParserRule(parent)) {
      if (isGroup(parent) || isAlternatives(parent) || isUnorderedGroup(parent)) {
        const index2 = parent.elements.indexOf(item);
        ruleName = index2.toString() + ":" + ruleName;
      }
      item = parent;
...
class MultiMap {
  ...
  delete(key, value) {
    if (value === void 0) {
      ...
      if (values2) {
        const index2 = values2.indexOf(value);
        if (index2 >= 0) {
          if (values2.length === 1) {
            this.map.delete(key);
          } else {
    ...
    for (const [propertyName, item] of Object.entries(node)) {
      if (Array.isArray(item)) {
        for (let index2 = 0; index2 < item.length; index2++) {
          const element = item[index2];
          if (isIntermediateReference(element)) {
            item[index2] = this.reviveReference(node, propertyName, root2, element, options);
          } else if (isAstNode(element)) {
            this.linkNode(element, root2, options, node, propertyName, index2);
          }
        }
...
class DefaultDocumentBuilder {
  ...
  onUpdate(callback) {
    ...
    return Disposable.create(() => {
      const index2 = this.updateListeners.indexOf(callback);
      if (index2 >= 0) {
        this.updateListeners.splice(index2, 1);
      }
    });
  }
  /**
...
function tokenize(context) {
  ...
  for (let i = 0; i < context.lines.length; i++) {
    ...
    const whitespaceEnd = skipWhitespace(line, index2);
    if (whitespaceEnd >= line.length) {
      ...
      tagRegex.lastIndex = index2;
      const tagMatch = tagRegex.exec(line);
      if (tagMatch) {
        const fullMatch = tagMatch[0];
        const value = tagMatch[1];
        const start = Position.create(currentLine, currentCharacter + index2);
        const end = Position.create(currentLine, currentCharacter + index2 + fullMatch.length);
        tokens.push({
          type: "tag",
          content: value,
          range: Range.create(start, end)
        });
        index2 += fullMatch.length;
        index2 = skipWhitespace(line, index2);
      }
      if (index2 < line.length) {
        const rest = line.substring(index2);
        const inlineTagMatches = Array.from(rest.matchAll(inlineTagRegex));
        tokens.push(...buildInlineTokens(inlineTagMatches, rest, currentLine, currentCharacter + index2));
      }
    }
    currentLine++;
    currentCharacter = 0;
...
function skipWhitespace(line, index2) {
  const match = line.substring(index2).match(nonWhitespaceRegex);
  if (match) {
    return index2 + match.index;
  } else {
    return line.length;
  }
}
...
function renderInlineTag(tag, content, options) {
  ...
  if (tag === "linkplain" || tag === "linkcode" || tag === "link") {
    ...
    let display = content;
    if (index2 > 0) {
      const displayStart = skipWhitespace(content, index2);
...
const CherryPicking = "CherryPicking";
const Commit = "Commit";
const Curve = "Curve";
const Edge = "Edge";
const Entry = "Entry";
...
const Packet = "Packet";
const PacketBlock = "PacketBlock";
const Pie = "Pie";
const PieSection = "PieSection";
const Radar = "Radar";
const Service = "Service";
const Direction = "Direction";
class MermaidAstReflection extends AbstractAstReflection {
  getAllTypes() {
    return [Architecture, Axis, Branch, Checkout, CherryPicking, Commit, Curve, Direction, Edge, Entry, GitGraph, Group, Info, Junction, Merge, Option2, Packet, PacketBlock, Pie, PieSection, Radar, Service, Statement];
  }
  computeIsSubtype(subtype, supertype) {
    switch (subtype) {
      case Branch:
...
class AbstractMermaidValueConverter extends DefaultValueConverter {
  runConverter(rule, input, cstNode) {
...
class AbstractMermaidTokenBuilder extends DefaultTokenBuilder {
...
function createGitGraphServices(context = EmptyFileSystem) {
  ...
  const GitGraph2 = inject(createDefaultCoreModule({ shared }), GitGraphGeneratedModule, GitGraphModule);
...
function createInfoServices(context = EmptyFileSystem) {
  const shared = inject(createDefaultSharedCoreModule(context), MermaidGeneratedSharedModule);
  const Info2 = inject(createDefaultCoreModule({ shared }), InfoGeneratedModule, InfoModule);
...
function createPacketServices(context = EmptyFileSystem) {
  const shared = inject(createDefaultSharedCoreModule(context), MermaidGeneratedSharedModule);
  const Packet2 = inject(createDefaultCoreModule({ shared }), PacketGeneratedModule, PacketModule);
...
function createPieServices(context = EmptyFileSystem) {
  const shared = inject(createDefaultSharedCoreModule(context), MermaidGeneratedSharedModule);
  const Pie2 = inject(createDefaultCoreModule({ shared }), PieGeneratedModule, PieModule);
...
function createArchitectureServices(context = EmptyFileSystem) {
  const shared = inject(createDefaultSharedCoreModule(context), MermaidGeneratedSharedModule);
  const Architecture2 = inject(createDefaultCoreModule({ shared }), ArchitectureGeneratedModule, ArchitectureModule);
...
function createRadarServices(context = EmptyFileSystem) {
  const shared = inject(createDefaultSharedCoreModule(context), MermaidGeneratedSharedModule);
  const Radar2 = inject(createDefaultCoreModule({ shared }), RadarGeneratedModule, RadarModule);
...
const initializers = {
  info: async () => {
    const { createInfoServices: createInfoServices2 } = await Promise.resolve().then(() => index$4);
    const parser = createInfoServices2().Info.parser.LangiumParser;
    parsers.info = parser;
  },
  packet: async () => {
    const { createPacketServices: createPacketServices2 } = await Promise.resolve().then(() => index$3);
    const parser = createPacketServices2().Packet.parser.LangiumParser;
    parsers.packet = parser;
  },
  pie: async () => {
    const { createPieServices: createPieServices2 } = await Promise.resolve().then(() => index$2);
    const parser = createPieServices2().Pie.parser.LangiumParser;
    parsers.pie = parser;
  },
  architecture: async () => {
    const { createArchitectureServices: createArchitectureServices2 } = await Promise.resolve().then(() => index$1);
    const parser = createArchitectureServices2().Architecture.parser.LangiumParser;
    parsers.architecture = parser;
  },
  gitGraph: async () => {
    const { createGitGraphServices: createGitGraphServices2 } = await Promise.resolve().then(() => index$5);
    const parser = createGitGraphServices2().GitGraph.parser.LangiumParser;
    parsers.gitGraph = parser;
  },
  radar: async () => {
    const { createRadarServices: createRadarServices2 } = await Promise.resolve().then(() => index);
